...
export default function StudioPage() {
  ...
  useEffect(() => {
    ...
    getAssignment(id)
      ...
      .catch((err) => setPageError(err.response?.data?.error || 'Assignment not found.'))
  ...
  const handleRunQuery = async () => {
    ...
    try {
      ...
      setQueryError(err.response?.data || { error: 'Network error. Is the backend running?' });
    } finally {
  ...
  if (pageLoading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh', flexDirection: 'column', gap: '1rem', color: '#94a3b8' }}>
  ...
  if (pageError) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '80vh', flexDirection: 'column', gap: '1rem', color: '#ef4444' }}>
        <p>❌ {pageError}</p>
        <button className="btn btn--secondary" onClick={() => navigate('/')}>← Back to Assignments</button>
      </div>
    );
  }
      ...
      <div className="studio__layout">
        ...
        <aside className="studio__left">
          ...
          <div className="studio__tabs">
            {['question', 'schema', 'data'].map((tab) => (
              <button
                key={tab}
                className={`studio__tab${leftTab === tab ? ' studio__tab--active' : ''}`}
                onClick={() => setLeftTab(tab)}
              >
                {tab === 'question' ? '📋 Question' : tab === 'schema' ? '🗂 Schema' : '🔢 Sample Data'}
              </button>
            ))}
          </div>
              ...
              <div className="studio__panel-body">
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                  <span className={`badge badge--${assignment.difficulty}`}>
                    {difficultyIcon[assignment.difficulty]} {assignment.difficulty}
                  </span>
                  {(assignment.tags || []).map((t) => (
                    <span key={t} className="assignment-card__tag">{t}</span>
                  ))}
                </div>
                ...
                {assignment.requirements?.length > 0 && (
                  <>
                    <p style={{ fontSize: '0.75rem', fontWeight: 600, color: '#6366f1', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>
                      Requirements
                    </p>
              ...
              <div className="studio__panel-body">
                {(assignment.tableSchemas || []).map((t) => (
                  <div key={t.tableName} className="studio__schema-table">
                    <div className="studio__schema-table-name">📁 {t.tableName}</div>
                    <div className="studio__schema-cols">
                      {t.columns.map((col) => (
                        <div key={col.name} className="studio__schema-col">
                          <span className="studio__schema-col-name">{col.name}</span>
                          <span className="studio__schema-col-type">{col.type}</span>
                          {col.description && (
                            <span className="studio__schema-col-desc">— {col.description}</span>
                          )}
                        </div>
                      ))}
                    </div>
        ...
        <div className="studio__right">
          ...
          <div className="studio__editor-area">
            ...
            <div className="studio__editor-wrap" onKeyDown={handleEditorKeyDown}>
              <Editor
                height="100%"
                defaultLanguage="sql"
                theme="vs-dark"
          ...
          {hint && (
            <div className="studio__hint">
              <div className="studio__hint-header">
                ...
                <button className="studio__hint-close" onClick={() => setHint(null)}>×</button>
              </div>
          ...
          <div className="studio__results">
            ...
            <div className="studio__results-body">
              <ResultsTable result={result} error={queryError} loading={queryLoading} />
            </div>
          </div>
